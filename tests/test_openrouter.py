import os
import pytest
from ai_sdk import openrouter, generate_text, stream_text, generate_object
...
# Skip all tests if OPENROUTER_API_KEY is missing
...
@pytest.mark.asyncio
async def test_openrouter_generate_text(model):
...
@pytest.mark.asyncio
async def test_openrouter_stream_text(model):
...
@pytest.mark.asyncio
async def test_openrouter_generate_object(model):
...
@pytest.mark.asyncio
async def test_openrouter_tool_call(model):
